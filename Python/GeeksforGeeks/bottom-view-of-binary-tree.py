...
def bottomView(root):
    ...
    while level:
        for _ in range(len(level)):
            node, vertical_level = level.pop(0)
            
            res[vertical_level] = node.data
            min_level = min(min_level, vertical_level)
            max_level = max(max_level, vertical_level)
            
            if node.left:
                level.append((node.left, vertical_level-1))
            if node.right:
                level.append((node.right, vertical_level+1))
    
    new_res = []
    for level_val in range(min_level, max_level+1):
        new_res.append(res[level_val])
